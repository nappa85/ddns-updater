...
async fn main() -> Result<(), ()> {
    ...
    let res = conn.query("SELECT value FROM metadata WHERE `key` = 'DEVICEAPI_HOST_WHITELIST'").await.map_err(|e| error!("Can't query MySQL: {}", e))?;
    ...
    for address in response.iter() {
        ...
        if !ips.contains(&s) {
            println!("DEVICEAPI_HOST_WHITELIST doesn't containts {}", s);
        }
    }
